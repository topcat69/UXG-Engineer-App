...
export async function syncIssueToMonday(supabase: AnySupabaseClient, issueId: string): Promise<void> {
  try {
    const { data: issue } = await supabase
      .from("issues")
      .select("severity, category, description, blocks_completion, status, created_at, job:jobs!issues_job_id_fkey(job_number)")
      .eq("id", issueId)
      .single();
    if (!issue || !issue.job) return;

    const itemName = buildIssueItemName(issue, issue.job);
    const columnValues = buildIssueColumnValues(issue, issue.job);
