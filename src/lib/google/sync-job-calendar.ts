...
export async function syncCalendarForJob(supabase: AnySupabaseClient, jobId: string): Promise<void> {
  try {
    const { data: job } = await supabase
      .from("jobs")
      .select(
        "id, job_number, scheduled_start, scheduled_end, calendar_event_id, description, job_type, status, assigned:users!jobs_assigned_to_fkey(name), job_details(job_information, sla_requirement_detail), job_equipment(model, serial), site:sites(name, address_line1, address_line2, town, postcode, access_notes, contact_name, contact_phone, latitude, longitude)",
      )
      .eq("id", jobId)
      .single();
