...
describe("parseProjectsCsv", () => {
  it("parses a valid row", () => {
    const csv = "name,start_date,end_date,status\nRollout 2026,2026-01-01,,active";
    const { rows, errors } = parseProjectsCsv(csv);
    expect(errors).toEqual([]);
    expect(rows).toEqual([{ name: "Rollout 2026", start_date: "2026-01-01", end_date: undefined, status: "active" }]);
  });
