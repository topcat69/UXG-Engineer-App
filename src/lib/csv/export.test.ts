...
describe("jobsToCsv", () => {
  it("produces a header row followed by one data row", () => {
    ...
    expect(lines[0]).toBe("job_number,status,job_type,priority,customer,site,project,assigned_to,scheduled_start");
  ...
  it("returns just a header line for an empty result set, not an error", () => {
    const csv = jobsToCsv([]);
    expect(csv.trim()).toBe("job_number,status,job_type,priority,customer,site,project,assigned_to,scheduled_start");
  });
});
