...
/**
 * Deliberately not linked from anywhere in the app (not even /login) — the
 * magic-link fallback exists only for superadmin (see GOOGLE_SSO_ENFORCED in
 * getCurrentUser()), so this page's URL itself is the only thing gating who
 * knows it's here. It still enforces the real check server-side; this is
 * just to stop non-superadmin staff from stumbling into a dead-end OTP flow
    ...
    <main className="flex min-h-screen items-center justify-center p-4">
      <Card className="w-full max-w-sm">
        <CardHeader>
          ...
          <CardDescription>Superadmin magic-link sign-in.</CardDescription>
        </CardHeader>
        <CardContent>
          {status === "sent" ? (
