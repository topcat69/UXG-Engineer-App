...
export default async function KbArticleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: article, error }, { data: categories }, { data: attachments }] = await Promise.all([
    supabase
      .from("kb_articles")
      .select(
        "*, category:kb_categories(id, name), author:users!kb_articles_author_id_fkey(name), reviewer:users!kb_articles_reviewed_by_fkey(name)",
      )
      .eq("id", id)
      .single(),
  ...
  return <ArticleDetail article={article} categories={categories ?? []} attachments={attachmentsWithUrls} />;
}
