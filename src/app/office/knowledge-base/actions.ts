...
/** RESTRICT, no cascade — a category still filed against by an article can't be silently deleted out from under it. */
...
export async function deleteCategory(categoryId: string): Promise<Result> {
  ...
  if (error) {
    if (error.code === "23503") {
      return { ok: false, message: "Can't delete — at least one article is still filed under this category." };
    }
...
export async function updateArticle(
  articleId: string,
  input: { title: string; body: string; categoryId: string; tags: string },
): Promise<ArticleResult> {
  const title = input.title.trim();
  const body = input.body.trim();
  if (!title) return { ok: false, message: "Title is required." };
  ...
  const { data, error } = await supabase
    .from("kb_articles")
    .update({ title, body, category_id: input.categoryId, tags: parseTags(input.tags) })
    .eq("id", articleId)
    .select("*")
    .single();
  if (error) return { ok: false, message: error.message };
