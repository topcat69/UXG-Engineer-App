...
export type DepreciationSearchParams = { category_id?: string; status?: string };
 ...
 *
 * Shared between /office/asset-register/depreciation (manager/superadmin)
 * and /finance/depreciation (finance/superadmin) — basePath supplies the
 * "back" and "clear filters" links for whichever surface rendered it.
 */
export async function AssetDepreciationReport({
  searchParams,
  basePath,
}: {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <Link href={basePath} className="text-sm underline-offset-2 hover:underline">
      ...
      <form className="flex flex-wrap items-end gap-2" method="get">
        ...
        {(categoryId || status) && (
          <Link href={`${basePath}/depreciation`} className="text-muted-foreground text-sm underline">
            Clear
          </Link>
        )}
      </form>
