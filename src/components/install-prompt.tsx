...
export function InstallPrompt() {
  ...
  if (deferredPrompt) {
    return (
      <div className="flex items-center justify-between gap-3 border-b bg-muted/40 px-4 py-2 text-sm">
        <span>Install UXG Engineer Job Scheduler for offline access on site.</span>
  ...
  if (ios) {
    return (
      <div className="flex items-center justify-between gap-3 border-b bg-muted/40 px-4 py-2 text-sm">
        <span>
          Install UXG Engineer Job Scheduler: tap <strong>Share</strong>, then <strong>Add to Home Screen</strong>.
        </span>
        <Button size="sm" variant="ghost" onClick={() => setDismissed(true)}>
          Dismiss
        </Button>
